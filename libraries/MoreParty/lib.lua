...
function Lib:init()
    ...
    Utils.hook(DarkEquipMenu, "init", function(orig, self)
        ...
        if not Kristal.getLibConfig("moreparty", "scroller") and not Kristal.getLibConfig("moreparty", "classic_mode") and #Game.party >= 4 or #Game.party == 4 then
    ...
    Utils.hook(DarkMenuPartySelect, "draw", function(orig, self)
        ...
        if not Kristal.getLibConfig("moreparty", "scroller") then
            ...
            local offset = math.sin(Kristal.getTime()*2) * 3
            
            if self.selected_party > self.shown then
                self.shown = self.shown + 1
            end
            if self.selected_party + 2 < self.shown then
                self.shown = self.shown - 1
            end
            
            if self.shown ~= 3 then
                Draw.draw(self.arrow, -12 - offset, 14)
            end
            if self.shown ~= #Game.party then
                Draw.draw(self.arrow, 162 + offset, 14, 0, -1, 1)
            end

            Draw.scissor(0, -18, 150, 64)
            ...
            for i,party in ipairs(Game.party) do
                ...
                local ox, oy = party:getMenuIconOffset()
                Draw.draw(Assets.getTexture(party:getMenuIcon()), (i-1)*50 + (ox*2) - (self.shown - 3) * 50, oy*2, 0, 2, 2)
            end
            if self.focused then
                local frames = Assets.getFrames("player/heart_harrows")
                Draw.setColor(Game:getSoulColor())
                Draw.draw(frames[(math.floor(self.heart_siner/20)-1)%#frames+1], (self.selected_party-1)*50 + 10  - (self.shown - 3) * 50, -18)
            end
        end
        Object.draw(self)
    end)
        ...
        Utils.hook(LightBattle, "shortActText", function(orig, self, text)
            ...
            local function displayShortActText()
                ...
                if advances >= 3 then
                    ...
                    local text_exhausted = not (t1 and t2 and t3) or #text == 0
                    local advance_to_next = not text_exhausted and displayShortActText
                    self.battle_ui.short_act_text_1:setText(t1 and "[ut_shake][shake:"..MagicalGlassLib.light_battle_shake_text.."]" .. t1 or "", advance_to_next)
                    self.battle_ui.short_act_text_2:setText(t2 and "[ut_shake][shake:"..MagicalGlassLib.light_battle_shake_text.."]" .. t2 or "", advance_to_next)
                    self.battle_ui.short_act_text_3:setText(t3 and "[ut_shake][shake:"..MagicalGlassLib.light_battle_shake_text.."]" .. t3 or "", advance_to_next)
            ...
            if not Game.battle.multi_mode or #Game.battle.party <= 3 then
                ...
                for index,battler in ipairs(Game.battle.party) do
                    if #Game.battle.party == 4 and not Kristal.getLibConfig("moreparty", "classic_mode") then
                        ...
                        love.graphics.rectangle("fill", x + 62, y, (small and 12 or 26) * 1.2 + 1, 21)
                        if current > 0 then
                            love.graphics.setColor(MagicalGlassLib.PALETTE["player_karma_health"])
                            love.graphics.rectangle("fill", x + 62, y, math.ceil((Utils.clamp(current, 0, max) / max) * (small and 12 or 26)) * 1.2 + 1, 21)
                            love.graphics.setColor(MagicalGlassLib.PALETTE["player_health"])
                            love.graphics.rectangle("fill", x + 62, y, math.ceil((Utils.clamp(current - karma, 0, max) / max) * (small and 12 or 26)) * 1.2 + 1 - (karma_mode and 1 or 0), 21)
                        end
